...
public class Graph4N2V<T> /*extends Graph */ implements IGraph {
    ...
    private final Graph<T> _G;

    /**
     * 构造函数
     */
    public Graph4N2V() {
        this._G = new Graph<>(false, false);        //默认为无向无权图
    }
    ...
    public boolean loadGraphFromEdgelistFile(String file, String split, boolean isDirected, boolean isWeighted) {
        return this._G.loadGraphFromEdgeListFile(file, split, isDirected, isWeighted);
    }
    ...
    public List<Integer> neighbors(int id) {
        Node<T> node = this._G.getNode(id);
        //nodeList.sort();
        return new ArrayList<>(node.getOutEdgeList().keySet());
    }
