...
public class Word2Vec<T> {
    ...
    public void init(String passageFileName, int i) {
        this.passags = new Passage<>(PassageStorage.File, passageFileName);        //从文件读取，泛型类型T应该为String
        if (this.wordType == WordType.Integer) {
            this.vocabulary = new Vocabulary<>(Vocabulary.WordType.Integer);
        } else if (this.wordType == WordType.String) {
            this.vocabulary = new Vocabulary<>(Vocabulary.WordType.String);
        }
    ...
    public void startTrainning() {
        ...
        int localIteratorNumber;            //当前迭代次数
        int c;
                        ...
                        for (int i = randomWindow; i < this.windowSize * 2 + 1 - randomWindow; i++) {            //遍历当前词的上下文
                            if (i != windowSize)                //取上下文中，除了当前词
                            {
                                ...
                                if (c >= sentence.length)
                                    continue;            //for (int i = c; i < this.windowSize * 2 + 1 - c; i++)
